...
const StoriesSection: React.FC<StoriesSectionProps> = ({ data }) => {
  return (
    <div className="min-h-screen flex flex-col justify-center items-center py-20 px-4 bg-muted/30">
      <motion.div
      ...
      <div className="max-w-5xl mx-auto w-full">
        {data.items.map((story, index) => (
          <motion.div
            ...
            className="mb-6 md:mb-10 max-w-[92vw] mx-auto md:max-w-none"
          >
            <Card className="backdrop-blur-sm bg-background/80 shadow-lg">
              <CardContent className="pt-4 pb-4 md:pt-8 md:pb-6">
                <div className="flex flex-col items-center text-center">
                  <svg 
                    className="w-8 h-8 md:w-12 md:h-12 text-primary/20 mb-2 md:mb-4" 
                    fill="currentColor" 
                    viewBox="0 0 32 32"
                  >
                    <path d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z" />
                  </svg>
                  <p className="text-sm md:text-lg lg:text-xl mb-2 md:mb-4 italic">{story.quote}</p>
